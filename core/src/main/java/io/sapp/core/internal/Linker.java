...
public class Linker {
    ...
    static String getPatchedLdLibraryPath(boolean use64bit, String[] extraPaths) {
        ...
        if (!haveLinkerNamespaces()) {
            ...
            paths.append("/superuser"); // for detection
    ...
    private static String getOriginalLdLibraryPath() {
        ...
        if (LD_LIBRARY_PATH.endsWith(":/superuser"))
            return null;

        if (LD_LIBRARY_PATH.contains(":/superuser:"))
            return LD_LIBRARY_PATH.substring(LD_LIBRARY_PATH.indexOf(":/superuser:") + ":/superuser:".length());
